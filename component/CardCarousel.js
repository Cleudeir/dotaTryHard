...
function CardCarousel({_match, _avg}) {
  ...
  return (
    mounted &&
    <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 10, minHeight: '100vh'}}>
      <div>
        <h1>See the last 30 Matches</h1>
      </div>
          ...
          <div key={match.match_id} >
            <Card className="text-center" style={{marginTop: 10, borderRadius: 0}} >
              <Card.Header >
                ...
                <div> Radiant <Button style={{cursor: 'default'}} variant="primary">{match.radiant_score}</Button> - <Button style={{cursor: 'default'}} variant="primary">{match.dire_score}</Button> Dire
                </div>
