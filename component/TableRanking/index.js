          ...
          { isData &&
            isData.map( ( player, i ) => (
              <tr key={ player.profile.account_id }>
                ...
                <td>{ player.kills.toFixed( 1 ) } <AvgCompare player={ player } type={ 'kills' } /></td>
                <td>{ player.deaths.toFixed( 1 ) } <AvgCompare player={ player } type={ 'deaths' } /> </td>
                <td>{ player.assists.toFixed( 1 ) } <AvgCompare player={ player } type={ 'assists' } /> </td>
                <td>{ player.last_hits.toFixed( 1 ) }<AvgCompare player={ player } type={ 'last_hits' } /> </td>
                <td>{ player.denies.toFixed( 1 ) }<AvgCompare player={ player } type={ 'denies' } /> </td>
                <td>{ player.gold_per_min.toFixed( 0 ) } <AvgCompare player={ player } type={ 'gold_per_min' } /> </td>
                <td>{ player.xp_per_min.toFixed( 0 ) }<AvgCompare player={ player } type={ 'xp_per_min' } /> </td>
                <td>{ player.hero_damage.toFixed( 0 ) } <AvgCompare player={ player } type={ 'hero_damage' } /> </td>
                <td>{ player.tower_damage.toFixed( 0 ) } <AvgCompare player={ player } type={ 'tower_damage' } /> </td>
                <td>{ player.hero_healing.toFixed( 0 ) } <AvgCompare player={ player } type={ 'hero_healing' } /> </td>
              </tr>
            ) ) }
        </tbody>
      </Table>
